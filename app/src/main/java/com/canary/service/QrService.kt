...
class QrService {
    ...
    fun generatePassphrase(): String = generateMnemonic().joinToString(" ")
    ...
    fun generateMnemonic(): List<String> {
        ...
        val checksumNibble = (hash[0].toInt() ushr 4) and 0x0F

        val buffer = ByteArray(17)
        System.arraycopy(entropy, 0, buffer, 0, 16)
        buffer[16] = (checksumNibble shl 4).toByte()
        ...
        repeat(12) {
            while (bitCount < 11) {
                bits = (bits shl 8) or (buffer[bufIdx].toInt() and 0xFF)
                bitCount += 8
                bufIdx++
            }
            val wordIndex = (bits ushr (bitCount - 11)) and 0x7FF
            bitCount -= 11
            words.add(WORDS[wordIndex])
        }
    ...
    fun mnemonicToEntropy(words: List<String>): ByteArray? {
        ...
        for (word in words) {
            val idx = WORD_INDEX[word] ?: return null
            indices.add(idx)
        }
        ...
        for (i in 0 until 12) {
            ...
            for (b in 10 downTo 0) {
                bits[i * 11 + (10 - b)] = ((idx ushr b) and 1) == 1
            }
        }
        ...
        var checksumNibble = 0
        for (b in 0 until 4) {
            checksumNibble = (checksumNibble shl 1) or (if (bits[128 + b]) 1 else 0)
        }

        val hash = MessageDigest.getInstance("SHA-256").digest(entropy)
        val expectedChecksum = (hash[0].toInt() ushr 4) and 0x0F

        if (checksumNibble != expectedChecksum) return null

        return entropy
    }

    fun encryptBackupData(data: ByteArray, mnemonic: List<String>): ByteArray {
        val entropy = mnemonicToEntropy(mnemonic) ?: throw IllegalArgumentException("Invalid mnemonic")
        val key = deriveKey(entropy)
    ...
    fun decryptBackupData(encryptedData: ByteArray, mnemonic: List<String>): ByteArray? {
        return try {
            val entropy = mnemonicToEntropy(mnemonic) ?: return null
            val key = deriveKey(entropy)
            val cipher = Cipher.getInstance("AES/GCM/NoPadding")
    ...
    private fun deriveKey(entropy: ByteArray): SecretKey {
        val spec = PBEKeySpec(entropy.joinToString("") { "%02x".format(it) }.toCharArray(),
            "canary-recovery".toByteArray(), 100000, 256)
    ...
    companion object {
        ...
        val WORD_INDEX = WORDS.mapIndexed { index, word -> word to index }.toMap()
    }
}
